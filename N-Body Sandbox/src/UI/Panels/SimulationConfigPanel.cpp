...
namespace NBody::UI {
    ...
    SimulationConfigPanel::SimulationConfigPanel(Core::AppState nextState, UIStorage& storage)
    ...
    void SimulationConfigPanel::draw(Core::MessageBus& messageBus) {
    ...
    void SimulationConfigPanel::drawPreviewSection(Core::MessageBus& messageBus) {

        const float windowWidth = ImGui::GetContentRegionAvail().x;
        const float scrollingAreaHeight = ImGui::GetContentRegionAvail().y - 80.0f;

        ImGui::PushID(static_cast<int>(Core::moduleCount));
        ...
        std::size_t generatorIdx = static_cast<std::size_t>(Core::Module::Generator);
    ...
    void SimulationConfigPanel::drawApplyAndRunButton(Core::MessageBus& messageBus) {
        const float windowWidth = ImGui::GetContentRegionAvail().x;
        ...
        if (ImGui::Button(SimulationConfigConfig::applyText)) {
            for (std::size_t moduleIdx = 0; moduleIdx < Core::moduleCount; ++moduleIdx) {
                ...
                std::size_t currentSubModuleCount = Core::subModuleCount[moduleIdx];
