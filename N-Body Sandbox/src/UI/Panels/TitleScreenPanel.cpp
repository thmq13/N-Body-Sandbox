...
void TitleScreenPanel::draw(MessageBus& messageBus) {
    setUpWindowAndStyle();
...
void TitleScreenPanel::drawTitleText(const ImVec2& windowSize) {
    ...
    ImGui::TextUnformatted(
        TitleScreenConfig::titleText
    );

    ImGui::SetWindowFontScale(
        TitleScreenConfig::defaultFontScale
    );
}

void TitleScreenPanel::drawStartButton(const ImVec2& windowSize, MessageBus& messageBus) {
    ImGui::SetWindowFontScale(
        TitleScreenConfig::buttonTextScale
    );
...
void TitleScreenPanel::drawExitButton(const ImVec2& windowSize, MessageBus& messageBus) {
    ImGui::SetWindowFontScale(
        TitleScreenConfig::buttonTextScale
    );
    ...
    if (ImGui::Button(TitleScreenConfig::exitButtonText)) {
        messageBus.publish(CmdExitApplication{});
    }
